    ...
    <div className={`flex flex-col lg:flex-row h-screen bg-slate-50 overflow-hidden ${tier === UserTier.FREE ? 'is-free-user' : ''}`}>
      ...
      {showPricing && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md overflow-y-auto">
           <div className="bg-white rounded-[3.5rem] shadow-2xl w-full max-w-5xl overflow-hidden grid grid-cols-1 md:grid-cols-2 animate-in zoom-in duration-300">
              <div className="p-12 lg:p-16 bg-slate-50 border-r flex flex-col justify-center">
                 <h3 className="text-3xl font-black mb-8">Plan Semilla <span className="text-slate-400 ml-2">$0</span></h3>
                 <ul className="space-y-4 mb-10 text-sm font-bold text-slate-600">
                    <li className="flex gap-4"><CheckCircle2 size={20} className="text-emerald-500" /> 5 Conciliaciones mensuales</li>
                    <li className="flex gap-4 opacity-30"><X size={20} /> Sin marcas de agua en actas</li>
                 </ul>
                 <button disabled className="w-full py-5 bg-slate-200 text-slate-400 rounded-2xl font-black uppercase text-xs tracking-widest">Tu Plan Actual</button>
              </div>
              <div className="p-12 lg:p-16 relative flex flex-col justify-center">
                 <button onClick={() => setShowPricing(false)} className="absolute top-10 right-10 text-slate-300 hover:text-slate-900"><X size={28}/></button>
                 <h3 className="text-4xl font-black mb-8">Auditor Pro <span className="text-indigo-600 ml-2">$25</span></h3>
                 <ul className="space-y-4 mb-10 text-sm font-bold text-slate-600">
                    <li className="flex gap-4"><Zap size={20} className="text-indigo-600" /> Conciliaciones ILIMITADAS</li>
                    <li className="flex gap-4"><Zap size={20} className="text-indigo-600" /> Historial persistente en la nube</li>
                    <li className="flex gap-4"><Zap size={20} className="text-indigo-600" /> Sello de certificación digital</li>
                 </ul>
                 <button onClick={() => window.open('https://stripe.com/pay/conciliai_pro', '_blank')} className="w-full py-6 bg-indigo-600 text-white rounded-3xl font-black text-xl shadow-2xl hover:bg-indigo-700 transition-all active:scale-95">
                   Suscribirse Pro <Zap size={20} fill="currentColor" />
                 </button>
              </div>
           </div>
        </div>
